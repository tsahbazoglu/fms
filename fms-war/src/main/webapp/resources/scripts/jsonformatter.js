...
function formatJson(){
    jsonTextElement =   document.getElementById('iceformContent:jsonText');
    jsonTextElement.value='telman';
}
...
function FormatTextarea() {
    ...
    var sJSON = jsonTextElement.value;
    if (sJSON.length > 0) {
        try {
            ...
            jsonTextElement.value=FormatJSON(oJSON);
        } catch(sError) {
            console.log("You are attempting to parse invalid JSON.\nuysError : "+sError);
        }
    } else {
         console.log("<Bono>I still haven't found what I'm looking for</Bono>");
    }
}
...
function FullSize() {
    ...
    $("#json").animate({
        height: ($("#json").height() + ($(window).height() - (iBottomPos - iTopPos) + 2))
    }, 500);
}
