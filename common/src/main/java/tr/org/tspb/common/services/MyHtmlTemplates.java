...
public class MyHtmlTemplates implements Serializable {
    ...
    public static void userDetailTemplate(DynamicHtml<HtmlUsrDetail> view, HtmlUsrDetail htmlUsrDetail) {
        view
                ...
                .dynamic(body -> body.span().attrStyle("font-weight:bold;color:black;").text("Ad : ").__().text(htmlUsrDetail.getFirstname()))
                .br().__()
                .dynamic(body -> body.span().attrStyle("font-weight:bold;color:black;").text("Soyad : ").__().text(htmlUsrDetail.getLastname()))
                .br().__()
                .dynamic(body -> body.span().attrStyle("font-weight:bold;color:black;").text("Eposta : ").__().text(htmlUsrDetail.getEmail()))
                .br().__()
                .br().__()
                .span().attrStyle("font-weight:bold;color:black;").text("Roller : ").__()
                .__()// div
                .dynamic(body -> body.text(htmlUsrDetail.getRolesTable()))
                .br().__()
                .br().__()
                .span().attrStyle("font-weight:bold;color:black;font-size:18px;").text("Yetkilendirildiği Kuruluşlar : ").__()
                .br().__()
