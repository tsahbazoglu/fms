...
public class CtrlService extends CommonSrv {
    ...
    public void crossCheck(Document filter) throws Exception {
        ...
        checkAndWriteControlResult(filter);
    ...
    public void checkAndWriteControlResult(Document filter) {
        Document eventList = formService.getMyForm().getEvents();
        if (eventList != null) {
        ...
        checkControlResult(filter);
    }

    public ControlResult checkControlResult(Document filter) {
            ...
            if (eventList != null) {
                for (String eventKey : eventList.keySet()) {
                    ...
                    if (appearFunction != null && !loginController.isUserInRole(formService.getMyForm().getMyProject().getAdminAndViewerRole())) {
                        ...
                        Object value = commandResult.get(RETVAL);
                        controlResult.setShouldSendForm(Boolean.TRUE.equals(value));
                    }

                    if (controlResult.isShouldSendForm()) {
                        Object gui = dbObject.get(GUI);

                        if (gui instanceof Code) {
                            Document commandResult = mongoDbUtil.runCommand(formService.getMyForm().getDb(), ((Code) gui).getCode(), filter, null);
                        ...
                        if (gui instanceof Document) {
                            ...
                            if (objectAction instanceof Code) {
                                ...
                                if ("START_DIALOG".equals(controlResult.getMyActionType())) {
                                    controlResult.setDialog(action.get("dialog").toString());
                                }

                                if (action.get("func") != null) {
                                    controlResult.setMyaction(action.get("func").toString());
                                }

                                if (action.get(JAVA_FUNC) != null) {
                                    controlResult.setJavaFunc(action.get(JAVA_FUNC).toString());
                                }
                            }
                            controlResult.setSuccessMessage(((Document) gui).get(SUCCESS_MESSAGE).toString());
                            controlResult.setFailMessage(((Document) gui).get(FAIL_MESSAGE).toString());
                        }
                    }
                }
            }
    ...
    public class ControlResult {
        ...
        private boolean shouldSendForm = false;
        ...
        public void setShouldSendForm(boolean shouldSendForm) {
            this.shouldSendForm = shouldSendForm;
        }
